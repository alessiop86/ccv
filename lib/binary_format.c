...
SBinaryTag* read_tag_from_file(const char* filename, int useMemoryMap) {
...
void deallocate_file_tag(SBinaryTag* fileTag, int useMemoryMap) {
  if (useMemoryMap) {
...
float get_float_from_dict(SBinaryTag* tag, const char* wantedKey) {
  SBinaryTag* valueTag = get_tag_from_dict(tag, wantedKey);
  assert(valueTag != NULL);
